...
class PPO_PIBB:
    ...
    def act(self, observation, expert_obs, history_obs, frequency_change=None, dt=None):
        if self.test:
            ...
            action, rw_ppo_diff_cpg, rw_ppo_noise, stand_phase = self.curricula.act_curriculum(observation, expert_obs, history_obs, self.PPO,
                                                                    self.PIBB, change_frequency=frequency_change, dt=dt)
            self.gamma_PPO = self.curricula.algorithm_curriculum.gamma

            return action, rw_ppo_diff_cpg, rw_ppo_noise, stand_phase
