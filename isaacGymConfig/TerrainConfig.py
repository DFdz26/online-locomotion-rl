...
class Terrain:
    ...
    def get_info_terrain(self, positions):
        level = torch.floor((positions[:, 0] - self.config.border_x) / (self.config.terrain_width + 0.1)).to(
            torch.uint8)
    ...
    def build_terrain(self):
        ...
        for i, dic in enumerate(self.terrain_list):
            ...
            for j in range(self.config.columns):
                info, heightfield[i * self.num_rows:(i + 1) * self.num_rows,
                      j * self.num_cols:(j + 1) * self.num_cols] = self.terrain_match[name](dic)
